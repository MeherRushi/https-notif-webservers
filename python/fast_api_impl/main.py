...
async def read_data_set_server_capabilities(file_path):
    if file_path.endswith(".xml"):
        return await parse_xml_capabilities(file_path)
    elif file_path.endswith(".json"):
        return await parse_json_capabilities(file_path)
    return False, False, []


async def parse_xml_capabilities(file_path):
    capabilities_data = []
    async with aiofiles.open(file_path, "rb") as xml_file:
...
async def parse_json_capabilities(file_path):
    capabilities_data = []
    async with aiofiles.open(file_path, "r") as json_file:
        data = await json_file.read()
        receiver_capabilities = json.loads(data).get("receiver-capabilities", {})
...
async def call_c_program(data_file_path, yang_model_path):
    process = await asyncio.create_subprocess_exec(
        "../../get_yang_files_and_validators/yang_validate",
        data_file_path,
        yang_model_path,
...
async def run_blocking_executable():
    return await asyncio.to_thread(call_c_program, data_file_path, yang_model_path)

@app.get("/capabilities")
async def get_capabilities(accept: Optional[str] = Header(None)):
    if not await run_blocking_executable():
        raise HTTPException(
...
async def post_notification(content_type: Optional[str] = Header(None)):
    json_capable, xml_capable, _ = await read_data_set_server_capabilities(data_file_path)
